...
normalTissues<-function(x)
{
  #return(grep("Normal",tcga$Pheno$histology,ignore.case=TRUE))
  a<-apply(matrix(x,nrow=1),1,tcgaSampleType)
  return(a == "Normal")
}

tumorTissues<-function(x)
{
  #return(grep("Carcinoma",tcga$Pheno$histology,ignore.case=TRUE))
  a<-apply(matrix(x,nrow=1),1,tcgaSampleType)
  return(a == "Tumor")
}

analysisSamples<-function(x)
{
  return(normalTissues(x) | tumorTissues(x))
}
...
{
  # This only differentiates tumors (-0) from normals (-1).  Why are some "control analytes" (-20)
  out<-substring(tcgaId,1,15)
  return(out)
}

standardizeTcgaIds<-function(ids)
{
  #print(paste0("Length of ids=",length(ids)))
  require("snow")
  pthreads <- detectCores()-3
  cl <- makeCluster(pthreads,type="SOCK")
  out <- parSapplyLB(cl,as.character(ids),standardizeTcgaId)
  stopCluster(cl)
  names(out) <- NULL
  return(out)
}
