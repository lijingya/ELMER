...
suppressWarnings({
  summary <- data.frame(
    nrow(readr::read_csv(paste0(root, "/getMethdiff.",direction,".probes.significant.csv"))),
    nrow(readr::read_csv(paste0(root,"/getPair.",direction, ".pairs.significant.csv"))),
...
suppressWarnings({
  p <- readr::read_csv(paste0(root,"/getMethdiff.",dir,".probes.csv"))
})
...
if(file.exists(file)) {
  ...
  suppressWarnings({
    .df <- readr::read_csv(paste0(root,"/getPair.",dir, ".pairs.significant.csv"))
  })
  ...
  if(file.exists(file)) {
    ...
    if(length(names(motifs.enriched)) > 0) {
      ...
      cat("\n### TF analysis\n")
      suppressWarnings({
        TF <- readr::read_csv(paste0(root,"/getTF.",dir,".significant.TFs.with.motif.summary.csv"))
      })
