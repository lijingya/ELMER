...
start site specified by the GECODE gene annotation. Thus, exactly 20 statistical 
...
9. portion: A number specifing the cut point for methylated and unmethylated. Default is 0.3.
10. diffExp: A logic. Default is FALSE. If TRUE, t test will be applied to test whether putative target gene are differentially expressed between two groups.
11. dir.out	A path specify the directory for outputs. Default is current directory
12. cores:	A interger which defines number of core to be used in parallel process. Default is 1: don't use parallel process.
13. label:	A character labels the outputs.
...
```{r, echo=TRUE, message=FALSE,warning=FALSE, fig.cap="Each scatter plot shows the methylation level of an example  probe cg19403323 in all LUSC samples plotted against the expression of one of  20 adjacent genes."}
scatter.plot(data,
...
schematic.plot(pair = Hypo.pair, data = data, byGene = "ENSG00000197965", save = FALSE)
```
