...
object can be generated by `createMultiAssayExperiment` function. In order to perform all analyses
...
When TCGA  data are used, the sample information will be automatically generated by `createMultiAssayExperiment` 
...
save this matrix as meth.rda since  `createMultiAssayExperiment` 
...
into `r Biocpkg("ELMER")`. It is recommended to save this matrix as RNA.rda since `createMultiAssayExperiment` 
...
`createMultiAssayExperiment`  function by specifying option TCGA=TRUE.

```{r}
library(MultiAssayExperiment)
data <- createMultiAssayExperiment(exp = GeneExp, met = Meth, genome = "hg19",TCGA = TRUE)
data
pData(data)[1:3,1:5]
...
phenotype.data <- data.frame(row.names = colnames(not.tcga.exp), samples = colnames(not.tcga.exp), group = c(rep("group1", ncol(GeneExp)/2),rep("group2", ncol(GeneExp)/2)))
data.hg19 <- createMultiAssayExperiment(exp = not.tcga.exp, met =  not.tcga.met, TCGA = FALSE, genome = "hg19", pData = phenotype.data)
data.hg19
...
phenotype.data <- data.frame(row.names = colnames(not.tcga.met), 
                             ...
                             group = c(rep("group1", ncol(Meth)/4),rep("group2", ncol(Meth)/4),rep("group3", ncol(Meth)/4),rep("group4", ncol(Meth)/4)))
data.hg38 <- createMultiAssayExperiment(exp = not.tcga.exp, met =  not.tcga.met, TCGA = FALSE, genome = "hg38", pData = phenotype.data)
data.hg38
...
It contains the above components and making a Multi Assay Experiment object by `createMultiAssayExperiment` function will keep each
...
expression), the column (samples) order should be same in the two matrixes. The `createMultiAssayExperiment`
...
```{r,eval=TRUE}
library(MultiAssayExperiment)
## createMultiAssayExperiment can take path as input.
data <- createMultiAssayExperiment(exp = "./ELMER.example/Result/LUSC/LUSC_RNA_refined.rda", 
                                   met = "./ELMER.example/Result/LUSC/LUSC_meth_refined.rda",
                                   genome = "hg19",TCGA = TRUE)
colnames(pData(data))
pData(data)[1:3,1:5]
...
nearGenes <-GetNearGenes(TRange=getMet(data)[Sig.probes,],
...
```{r, echo=TRUE, message=FALSE, fig.cap="Each scatter plot shows the methylation level of an example  probe cg19403323 in all LUSC samples plotted against the expression of one of  20 adjacent genes."}
scatter.plot(data,
...
scatter.plot(data,
             byPair=list(probe=c("cg19403323"),gene=c("ENSG00000204518")), 
