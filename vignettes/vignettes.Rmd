...
#Example file download from URL: "https://www.dropbox.com/s/mxf2u1wcauenx8m/ELMER.example.zip
URL <- "https://www.dropbox.com/s/mxf2u1wcauenx8m/ELMER.example.zip?raw=1"
if(Sys.info()["sysname"] == "Windows") mode <- "wb" else  mode <- "w"
downloader::download(URL,destfile = "ELMER.example.zip", mode= mode)
utils::unzip("ELMER.example.zip")
...
create the groups Tumor and Normal the same way the ELMER version 1.0 did, 
...
pData(data)[pData(data)$sample == "TCGA-43-3394-11A",1:8]
...
library(MultiAssayExperiment)
## createMAE can take path as input.
data <- createMAE(exp = "./ELMER.example/Result/LUSC/LUSC_RNA_refined.rda", 
                  met = "./ELMER.example/Result/LUSC/LUSC_meth_refined.rda",
                  save = FALSE,
                  met.platform = "450K",
                  genome = "hg19",
                  TCGA = TRUE)
...
head(sig.diff)   ## significantly hypomethylated probes
...
This filter is importante as true promoters and enhancers usually have a pretty low 
...
After, for each enhancer probe with differential methylation and passing this filter, the closest 10 upstream 
...
1. data: A multiAssayExperiment with DNA methylation and Gene Expression data. See createMAE function.
2. nearGenes:	Can be either a list containing output of GetNearGenes function or path of rda file containing output of GetNearGenes function.
3. percentage:	A number ranges from 0 to 0.5 specifying the percentage of samples used to link probes to genes. Default is 0.2.
4. permu.size:	A number specify the times of permuation. Default is 10000.
5. permu.dir	A path where the output of permutation will be saved
6. calculate.Pe: A logic. If TRUE (default) also execute the permutation step, if FALSE only uses the adjusted raw p-value to select the significant pairs.
7. pvalue: A number specify the raw p-value cutoff for defining signficant pairs.  Default is 0.05. It will select the significant P value (adjusted P value by BH) cutoff before calculating the empirical p-values.
8. Pe: A number specify the empirical p-value cutoff for defining signficant pairs. Default is 0.001. Only used if calculate.empirical.p is TRUE.
9. filter.probes Should filter probes by selecting only probes that have at least
   a certain number of samples below and above a certain cut-off. 
   See `preAssociationProbeFiltering` function.
10.  filter.portion: A number specifing the cut point for methylated and unmethylated. Default is 0.3.
11. filter.percentage:  Minimun percentage of samples to be considered in methylated and unmethylated 
for the filter.portion option. Default 5\%. See `preAssociationProbeFiltering` function.
12. diffExp: A logic. Default is FALSE. If TRUE, t test will be applied to test whether putative 
target gene are differentially expressed between two groups. If true two columns will be added
to the output log2FC.groupA.vs.groupB (positive values means gene has higher 
expression in group A compared to group B) and groupA.vs.groupB.diff.pvalue with the significance of the test (not adjusted).
13. group.col. A column defining the groups of the samples to be used if diffExp is TRUE. 
You can view the  available columns using: colnames(MultiAssayExperiment::pData(data)).
14. dir.out	A path specify the directory for outputs. Default is current directory
15. cores:	A interger which defines number of core to be used in parallel process. Default is 1: don't use parallel process.
16. label:	A character labels the outputs.
...
nearGenes <- GetNearGenes(data = data, probe = Sig.probes, cores = detectCores()/2)

## Identify significant probe-gene pairs
Hypo.pair <- get.pair(data = data,
                      nearGenes = nearGenes,
                      permu.dir = "./ELMER.example/Result/LUSC/permu",
                      permu.size = 200,
                      pvalue = 0.01,
                      calculate.Pe = TRUE,
                      Pe = 0.01,
                      probe.filtering = TRUE, # See preAssociationProbeFiltering function
...
The build in data Probes.motif is generated using HOMER with a p-value < 1e–4 
...
weight matrices (PWMs). HOCOMOCO offers 540 PMWs each has a quality rating from A to D where 
...
```{r, eval=TRUE, fig.cap="The schematic plot shows probe colored in blue and the location of nearby 20 genes. The genes significantly linked to the probe  were shown in red.", message=FALSE, warning=FALSE}
