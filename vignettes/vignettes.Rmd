...
data <- createMAE(exp = GeneExp, 
                  ...
                  TCGA = TRUE)
data
datatable(as.matrix(pData(data)[,c("patient","definition","TN")]),
          options = list(scrollX = TRUE, keys = TRUE, pageLength = 5), 
          rownames = TRUE)
...
data.hg38 <- createMAE(exp = not.tcga.exp, 
                       ...
                       pData = phenotype.data)
data.hg38
datatable(as.matrix(pData(data.hg38)[1:20,]),
          options = list(scrollX = TRUE, keys = TRUE, pageLength = 5), 
          rownames = TRUE)
```
...
datatable(as.matrix(pData(data)[pData(data)$sample == "TCGA-43-3394-11A",]),
          options = list(scrollX = TRUE, keys = TRUE, pageLength = 5), 
          rownames = TRUE)
...
If it is "hypo", `get.diff.meth` function will identify all significantly hypomethylated CpG sites in group 1 compared to group 2; 
If "hyper", `get.diff.meth` function will identify all significantly hypermethylated CpG sites in group 1 compared to group 2;
* **group.col**  A column defining the groups of each sample. You can view the available columns using: `colnames(MultiAssayExperiment::pData(data))`.
...
* **percentage** 	A number ranges from 0 to 1 specifying the percentage of samples from group1 and group2 that are used to identify the differential methylation. 
Default is 0.2 because we did not expect all cases to be from a single molecular subtype. ***But, If you are working with molecular subtypes please set it to 1.***
* **test** Statistical test to be used. Options: t.test (DEFAULT), wilcox.test
* **pvalue** A number that specifies the significant P value (adjusted P value by BH) cutoff for selecting significant hypo/hyper-methylated probes. Default is 0.01
...
datatable(as.matrix(pData(data)[1:5,]), 
          options = list(scrollX = TRUE, keys = TRUE, pageLength = 5), 
          rownames = TRUE)
...
After, for each enhancer probe with differential methylation and passing 
...
4. permu.size:	A number specify the times of permutation. Default is 10000.
5. permu.dir	A path where the output of permutation will be saved
6. pvalue: A number specify the raw p-value cutoff for defining signficant pairs.  Default is 0.05. It will select the significant P value (adjusted P value by BH) cutoff before calculating the empirical p-values.
7. Pe: A number specify the empirical p-value cutoff for defining signficant pairs. Default is 0.001. Only used if calculate.empirical.p is TRUE.
8. filter.probes Should filter probes by selecting only probes that have at least
   a certain number of samples below and above a certain cut-off. 
   See `preAssociationProbeFiltering` function.
9.  filter.portion: A number specifing the cut point for methylated and unmethylated. Default is 0.3.
10. filter.percentage:  Minimun percentage of samples to be considered in methylated and unmethylated 
for the filter.portion option. Default 5\%. See `preAssociationProbeFiltering` function.
11. diffExp: A logic. Default is FALSE. If TRUE, t test will be applied to test whether putative 
...
12. group.col. A column defining the groups of the samples to be used if diffExp is TRUE. 
You can view the  available columns using: colnames(MultiAssayExperiment::pData(data)).
13. dir.out	A path specify the directory for outputs. Default is current directory
14. cores:	A interger which defines number of core to be used in parallel process. Default is 1: don't use parallel process.
15. label:	A character labels the outputs.
