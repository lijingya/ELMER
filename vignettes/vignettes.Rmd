...
6. calculate.empirical.p: Execute the permutation step or use the adjusted raw p-value to select the significant pairs.
7. pvalue:	A number specify the pvalue cutoff for defining signficant pairs. Default is 0.001. 
If calculate.empirical.p is TRUE it will be the empirical pvalue cutoff as explained before. 
Otherwise it will select the significant P value (adjusted P value by BH) cutoff. 
8. portion: A number specifing the cut point for methylated and unmethylated. Default is 0.3.
9. diffExp: A logic. Default is FALSE. If TRUE, t test will be applied to test whether putative target gene are differentially expressed between two groups.
10. dir.out	A path specify the directory for outputs. Default is current directory
11. cores:	A interger which defines number of core to be used in parallel process. Default is 1: don't use parallel process.
12. label:	A character labels the outputs.
