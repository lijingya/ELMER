---
title: "3.6 - TCGA.pipe: Running ELMER for TCGA data in a compact way"
...
`TCGA.pipe` is a function for easily downloading TCGA data and performing all the analyses in ELMER. For illustration purpose, we skip the downloading step. 
...
We add in `TCGA.pipe` function (download step) the option to identify mutant samples to perform WT vs Mutant analysis. It will download open MAF file from GDC database, select a gene and identify the which are the mutant samples based on the following cliassification:
c("#008B00", "#FFFFFF", "#FFFFFF")
The  classification below was used, but it can be changed using the atgument `mutant_variant_classification`.
